...
class DataBridge:
    ...
    def __init__(self):
        ...
        self.longterm_dir = os.path.join(self.base_dir, "Strategy_Value_LongTerm")
        self.shortterm_dir = os.path.join(self.base_dir, "Strategy_Event_ShortTerm")

    def get_longterm_weights(self) -> pd.DataFrame:
        """读取长线权重配置"""
        weights_file = os.path.join(self.longterm_dir, "output_weights.csv")
    ...
    def get_shortterm_signals(self) -> dict:
        """读取短线信号"""
        signals_file = os.path.join(self.shortterm_dir, "daily_signals.json")
    ...
    def get_sector_heat_history(self) -> pd.DataFrame:
        """读取板块热度历史"""
        history_file = os.path.join(self.shortterm_dir, "sector_heat_history.csv")
